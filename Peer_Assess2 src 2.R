...
#' 
## ----Process data to find highest economic impact per types of events, results="hide", cache=TRUE----

## Convert all $ values to Billion's of $
# for (i in 1: dim(storm_data)[1]) {
#   
#   if (as.integer(i/10000)*10000==i) {print (i)}
#   
...
#     storm_data[i, "Cost"] <- storm_data[i, "PROPDMG"] * 1
#     }
#   
#   if (storm_data[i, "CROPDMGEXP"] == "K") {storm_data[i, "Cost"] <- storm_data[i, "CROPDMG"] / 1000000}
#   else if (storm_data[i, "CROPDMGEXP"] == "M") {storm_data[i, "Cost"] <- storm_data[i, "CROPDMG"] * 1000}
#   else if (storm_data[i, "CROPDMGEXP"] == "B") {storm_data[i, "Cost"] <- storm_data[i, "CROPDMG"] * 1}
#   
# }
...
#' 
#' As can be seen above - HURRICANE/TYPHOON has the highest economic impact, with STORM SURGE and TORNADO close to it.
#' 
#' 
